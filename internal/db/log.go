...
func (db *LogDB) GetLog(hash string) (*engine.Log, error) {
	...
	err := row.Scan(&log.Hash, &log.TxHash, &log.CreatedAt, &log.UpdatedAt, &log.Nonce, &log.To, &value, &log.Data, &log.ExtraData, &log.Status)
	if err != nil {
		return nil, err
	}
...
func (db *LogDB) GetAllPaginatedLogs(contract string, signature string, maxDate time.Time, topics engine.Topics, limit, offset int) ([]*engine.Log, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&log.Hash, &log.TxHash, &log.CreatedAt, &log.UpdatedAt, &log.Nonce, &log.To, &value, &log.Data, &log.ExtraData, &log.Status)
		if err != nil {
			return nil, err
		}
...
func (db *LogDB) GetAllNewLogs(contract string, signature string, fromDate time.Time, topics engine.Topics, limit, offset int) ([]*engine.Log, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&log.Hash, &log.TxHash, &log.CreatedAt, &log.Nonce, &log.To, &value, &log.Data, &log.ExtraData, &log.Status)
		if err != nil {
			return nil, err
		}
