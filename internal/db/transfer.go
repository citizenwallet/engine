...
func NewTransferDB(db, rdb *pgxpool.Pool, name string) (*TransferDB, error) {
	txdb := &TransferDB{
		suffix: name,
		db:     db,
		rdb:    rdb,
...
func (db *TransferDB) AddTransfer(tx *engine.Transfer) error {
...
func (db *TransferDB) AddTransfers(tx []*engine.Transfer) error {
...
func (db *TransferDB) ReconcileTxHash(tx *engine.Transfer) error {
	...
	// insert or update the confirmed transfer
	_, err = db.db.Exec(db.ctx, fmt.Sprintf(`
	INSERT INTO t_transfers_%s (hash, tx_hash, token_id, created_at, from_to_addr, from_addr, to_addr, nonce, value, data, status)
...
func (db *TransferDB) ReconcileTx(txHash, hash string, nonce int64) error {
	...
	`, db.suffix), txHash, nonce, string(engine.TransferStatusSuccess), hash)
...
func (db *TransferDB) GetTransfer(hash string) (*engine.Transfer, error) {
	var transfer engine.Transfer
	var value string
...
func (db *TransferDB) GetAllPaginatedTransfers(tokenId int64, maxDate time.Time, limit, offset int) ([]*engine.Transfer, error) {
...
func (db *TransferDB) GetPaginatedTransfers(tokenId int64, addr string, maxDate time.Time, limit, offset int) ([]*engine.Transfer, error) {
	transfers := []*engine.Transfer{}
...
func (db *TransferDB) GetAllNewTransfers(tokenId int64, fromDate time.Time, limit, offset int) ([]*engine.Transfer, error) {
	transfers := []*engine.Transfer{}
...
// GetNewTransfers returns the transfers for a given from_addr or to_addr from a given date
func (db *TransferDB) GetNewTransfers(tokenId int64, addr string, fromDate time.Time, limit, offset int) ([]*engine.Transfer, error) {
	transfers := []*engine.Transfer{}
...
func (db *TransferDB) GetProcessingTransfers(limit int) ([]*engine.Transfer, error) {
	...
	rows, err := db.rdb.Query(db.ctx, fmt.Sprintf(`
		...
		ORDER BY created_at DESC
		LIMIT $5
		`, db.suffix, db.suffix), engine.TransferStatusPending, fromDate, engine.TransferStatusSending, fromDate, limit)
	if err != nil {
		if err == pgx.ErrNoRows {
			return transfers, nil
		}
...
func (db *TransferDB) UpdateTransfersWithDB(txs []*engine.Transfer) ([]*engine.Transfer, error) {
	if len(txs) == 0 {
		return txs, nil
	}
	...
	mtxs := map[string]*engine.Transfer{}
	for _, tx := range txs {
		mtxs[tx.Hash] = tx
	}
